...
impl TheoremFrame {
    pub fn new(formula: Formula, depth: usize, position: usize, annotation: String, rule_num: u8, scope: usize) -> TheoremFrame {
        TheoremFrame{ formula, depth, position, annotation, rule_num, scope }
    }
}
...
impl Deduction {
    ...
    fn push_new(&mut self, theorem: Formula, annotation: String, rule_num: u8) {
        if NOISY {
            if rule_num == 10 {
    ...
    /// Dump the entire vector of TheoremFrame.
    ...
    pub fn add_axiom(&mut self, premise: &Formula) -> Result<(),LogicError> {
        if self.axioms.contains(&premise) {
            self.push_new( premise.clone(), "axiom".to_string(), 0 );
        } else {
    ...
    pub fn specification<T: Term>(&mut self, n: usize, var: &Variable, replacement: &T) -> Result<(),LogicError> {
    ...
    pub fn existence<T: Term>(&mut self, n: usize, term: &T, var: &Variable) -> Result<(),LogicError> {
    ...
    pub fn supposition(&mut self, premise: Formula) -> Result<(),LogicError> {
        ...
        self.depth += 1;
        self.push_new( premise, "supposition".to_string(), 10 );
        Ok(())
    }
    ...
    pub fn induction(&mut self, var: &Variable, base: usize, general: usize) -> Result<(),LogicError> {
